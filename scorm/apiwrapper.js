...
function LMSGetValue(name)
{
	WriteToDebug("....In LMSGetValue for " + name.toString());
	if (bFinishDone == false)
	{
		var api = getAPIHandle();
		if (api == null)
		...
		else
		{
			...
			SCORM_SetErrorInfo();
			WriteToDebug("...." + name.toString() + " = " + value.toString());
			if (value == "")
			{
				var err =  ErrorHandler();
...
function LMSSetValue(name, value) 
{   
	WriteToDebug("....In LMSSetValue for " + name.toString() + " = " + value.toString());
   if (bFinishDone == false)
   {
       var api = getAPIHandle();
       if (api == null)
       ...
       else
       {
		  ...
		  SCORM_SetErrorInfo();
		  WriteToDebug("....Result after setting value for " + name.toString() + " = " + setRet.toString());
		  if (setRet != "true")
		  {
	          var err = ErrorHandler();
		  }
...
intSCORMError=api.LMSGetLastError();
strSCORMErrorString=api.LMSGetErrorString(intSCORMError);
strSCORMErrorDiagnostic=api.LMSGetDiagnostic("");
